...
### Tiếng Việt:
Ứng dụng này được phát triển cho mục đích học tập và thực tập. **Không đảm bảo tính tin cậy, tính sẵn sàng hay tính chính xác** cho mục đích thương mại hoặc sử dụng thực tế.
- Người dùng tự chịu trách nhiệm về việc sử dụng ứng dụng này.
- Nhà phát triển không chịu trách nhiệm về bất kỳ thiệt hại, mất mát hay vấn đề nào phát sinh từ việc sử dụng ứng dụng.
- Dữ liệu hiển thị có thể là dữ liệu mẫu (mock) hoặc từ API thực tế tùy theo cấu hình, hãy kiểm tra kỹ trước khi tin cậy.
- Không sử dụng cho các hệ thống quan trọng hoặc môi trường sản xuất mà không có kiểm tra và xác minh đầy đủ.

### English:
This application is developed for **educational and internship purposes only**. It is provided "as is" without any warranty of reliability, availability, or accuracy for commercial or real-world use.
- Users are solely responsible for using this application.
- The developers are not liable for any damages, losses, or issues arising from using this application.
- Displayed data may be mock data or from real API depending on configuration; verify carefully before relying on it.
- Do not use for critical systems or production environments without thorough testing and validation.
